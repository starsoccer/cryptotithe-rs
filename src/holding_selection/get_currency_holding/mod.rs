...
use crate::{holding, trade, method};
mod cost_first_out;
...
pub fn get_currency_holding (currency_holdings: &Vec<holding::CurrencyHolding>, method: method::Method, trade: trade::Trade) -> usize {
    //holdings.0.get_mut(&trade.sold_currency).unwrap();
    match method {
        method::Method::LTFO => lowest_tax_first_out::lowest_tax_first_out(trade, currency_holdings),
        method::Method::HTFO => highest_tax_first_out::highest_tax_first_out(trade, currency_holdings),
...
mod tests {
    ...
    fn get_currency_holding_fifo() {
        ...
        let result = get_currency_holding(currency_holdings, method::Method::FIFO, trades[0].clone());

        assert_eq!(result, 0);
        assert_eq!(&currency_holdings[result], currency_holdings.first().unwrap());
    }
    ...
    fn get_currency_holding_lifo() {
        ...
        let result = get_currency_holding(currency_holdings, method::Method::LIFO, trades[0].clone());

        assert_eq!(result, currency_holdings.len() - 1);
        assert_eq!(&currency_holdings[result], currency_holdings.last().unwrap());
    }
    ...
    fn get_currency_holding_hcfo() {
        ...
        let result = get_currency_holding(currency_holdings, method::Method::HCFO, trades[0].clone());

        let mut highest_cost_holding_index = 0;
        for (index, current_currency_holding) in currency_holdings.iter().enumerate() {
            if current_currency_holding.rate_in_fiat > currency_holdings[highest_cost_holding_index].rate_in_fiat {
                highest_cost_holding_index = index;
            }
        }

        assert_eq!(result, highest_cost_holding_index);
        assert_eq!(currency_holdings[result], currency_holdings[highest_cost_holding_index]);
    }
    ...
    fn get_currency_holding_lcfo() {
        ...
        let result = get_currency_holding(currency_holdings, method::Method::LCFO, trades[0].clone());

        let mut lowest_cost_holding_index = 0;
        for (index, current_currency_holding) in currency_holdings.iter().enumerate() {
            if current_currency_holding.rate_in_fiat < currency_holdings[lowest_cost_holding_index].rate_in_fiat {
                lowest_cost_holding_index = index;
            }
        }

        assert_eq!(result, lowest_cost_holding_index);
        assert_eq!(currency_holdings[result], currency_holdings[lowest_cost_holding_index]);
    }
    ...
    fn get_currency_holding_htfo_short_term() {
        ...
        let htfo_result = get_currency_holding(currency_holdings, method::Method::HTFO, trades[0].clone());
        let lcfo_result = get_currency_holding(currency_holdings, method::Method::LCFO, trades[0].clone());
    ...
    fn get_currency_holding_htfo_long_term() {
        let holdings = mocks::mock_holdings(1, 10, Some(mocks::now_u64() - YEAR_IN_MILLISECONDS * 3), Some(mocks::now_u64() - YEAR_IN_MILLISECONDS * 2));
        let currency = holdings.0.keys().collect::<Vec<&String>>()[0];
        let currency_holdings = holdings.0.get(currency).unwrap();
        let mut trades = mocks::mock_trades(1, 123456768,holdings.clone(), false);
        trades[0].sold_currency = currency.into();
    ...
    fn get_currency_holding_htfo_short_and_long_term() {
        let mut holdings = mocks::mock_holdings(1, 10, Some(mocks::now_u64() - YEAR_IN_MILLISECONDS * 3), None);
    ...
    fn get_currency_holding_ltfo_short_term() {
        ...
        let ltfo_result = get_currency_holding(currency_holdings, method::Method::LTFO, trades[0].clone());
        let hcfo_result = get_currency_holding(currency_holdings, method::Method::HCFO, trades[0].clone());
